...
const AffecterCritere = () => {
  ...
  // Affectation des critères aux fichiers sélectionnés
  const handleUpdate = () => {
    try {
      setIsLoading(true);
      ...
      const tblTargetedFiles = tbl.filter((elem) =>
        tblSelect.includes(elem.file)
      );

      const storeCriterias = async () => {
        console.log("storeCriterias");
        const response = await axios.put(
          "http://localhost:4000/criterias-store",
          {
            arrayCheck: tblId,
            arrayTargetedFiles: tblTargetedFiles,
          }
        );
      };
      storeCriterias();
  ...
  const handleArchive = () => {
    ...
    try {
      const archive = async () => {
        const response = await axios.put("http://localhost:4000/archive", {
          arrayFile: tblTargetedFiles,
        });
      };
      archive();
  ...
  return isLoading ? (
    ...
    <div className="container">
      <h1>Classer - Etape 2 : affectation de critères</h1>

      <div className="aff-cri-button-header">
        <input
          type="button"
...
export default AffecterCritere;
